...
TapSignOn::TapSignOn()
{
     ...
     try
    {
        ...
        nn_bind(TapSignOn::AdminSock,"tcp://192.168.100.222:8011");
...
void TapSignOn::BindConnection()
{
    ...
    try
    {
		...
		const string servAddress =  "127.0.0.1";	// First arg: server address
...
*/
					...
					case 20222:
					{
					 ...
					 boost::mutex::scoped_lock lockhash(the_Hashmutex);
                        ...
                        _PH =  _PFHashMap.GetRecordByOrder(_OrderResp.ResponseOrderNumber,ntohl(_OrderResp.RemainingVolume));

                        if(_PH.PF==0)
                        {
                            cout << "Break  From PF ZERO 20222 OrderNumber "<< boost::lexical_cast<string>(_OrderResp.ResponseOrderNumber) << " Token "<< htonl(_OrderResp.Token) << " Price "<< htonl(_OrderResp.Price)<< " Volume "<<htonl(_OrderResp.OriginalVolume) << " B/S "<< htons(_OrderResp.Buy_SellIndicator)  <<endl;
                        }
                        if(_PH.PF>0)
                        {
                            memcpy(Data, &_PH.SubscriptionTag,8);
				...
				/*"Hut@7895"*/
						  pkt.SeqNo = 1;
						  LastSeq = pkt.SeqNo;
						  MS_SIGN_ON_2300 Login = pkt.LoginPacket_2300(28823,"Nn@33333", "","12468", 4,93800, 0, "1234567","DIVYA PORTFOLIO PVT LTD");
